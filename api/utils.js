...
module.exports = {
  // analyzes peer's fees and returns an array of messages with importance
  // importance grades: normal, warning, urgent
  analyzeFees(peerName, peerId, localFee, remoteFee) {
    ...
    const enforceMaxPpm = config.rebalancer.enforceMaxPpm;
    const buffer = constants.rebalancer.buffer;

    let array = [];

    let intro = 'noticed a potential issue with [outbound] ' + peerName + '. current fees: local { base: ' + localFee.base + ', ppm: ' + localFee.rate + '} remote { base: ' + remoteFee.base + ', ppm: ' + remoteFee.rate + ' }.';

    if (enforceMaxPpm) {
      array.push({
        importance: normal,
        message: 'enforceMaxPpm is enabled for ' + peerName + '. current fees: local { base: ' + localFee.base + ', ppm: ' + localFee.rate + '} remote { base: ' + remoteFee.base + ', ppm: ' + remoteFee.rate + ' }.'
      })
      return array;
    }
    ...
    if (xx >= 10) {
      ...
      // action
      if (array.length === 0) array.push({ importance: normal, message: intro });
      array.push({
        importance: warning,
        message: 'current local fee exceeds the max ppm of ' + maxPpm + ' by more than ' + xx + 'x. assuming default max ppm of ' + maxPpm + '. revisit your fees'
      })
      return array; 
    }
    if (xx >= 2) {
      ...
      array.push({
        importance: warning,
        message: 'peer\'s local fee exceeds the max ppm of ' + maxPpm + ' by more than ' + xx + 'x. this means that rebalances will likely be more expensive than your set max. revisit your fees just in case'
      })
    }
    if (local < remote + buffer) {
      ...
      if (local < remote) {
        array.push({
          importance: urgent,
          message: 'negative rebalance margin of ' + (local - remote) + ' sats. this means that rebalances into this peer are not profitable'
        })
      } else {  // remote < local < remote + buffer
        array.push({
          importance: urgent,
          message: 'not enough of a buffer between local and remote fees: ' + (local - remote) + ' vs ' + buffer + ' recommended. this means that rebalances have less of a chance to go through'
        })
      }
      ...
      if (xx >= 10) {
        array.push({
          importance: urgent,
          message: 'peer\'s remote fee is too high at the moment, it exceeds your max ppm by more than ' + xx + 'x. consider waiting until the peer adjusts their fees to something more reasonable. in the meantime, rebalances will automatically be placed on hold'
        })
        return array; // skip to the next iteration
      } 
      if (xx >= 2) {
        array.push({
          importance: normal,
          message: 'peer\'s remote fee is quite high, it exceeds the max ppm by more than ' + xx + 'x. consider setting peer\'s ppm to ' + (remote + buffer) + ', but proceed with caution. an alternative is to wait until the peer adjusts their fees'
        })
      } else {
        array.push({
          importance: normal,
          message: 'to resolve this, consider increasing peer\'s ppm to ' + (remote + buffer)
        })
      }
    }
    return array;
